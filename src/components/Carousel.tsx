...
export default function Carousel({children, }: ICarousel): JSX.Element {
  ...
  return (
    <div className={classes["slider-parent"]}>
        <button className={classes['button-left-position1']} onClick={leftClickHandler} disabled={!hasPrevSlide}>&lt;</button>
      {Children.map(children, (item, index) => {
        ...
        <button className={classes['button-right-position1']} onClick={rightClickHandler} disabled={!hasNextSlide}>&gt;</button>
    </div>
  );
}
